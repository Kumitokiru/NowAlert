...
def get_bfp_trends(time_filter='weekly'):
    try:
        bfp_alerts = [a for a in alerts if a.get('role') == 'bfp' or a.get('assigned_municipality')]
...
def get_bfp_distribution(time_filter='weekly'):
    try:
        bfp_alerts = [a for a in alerts if a.get('role') == 'bfp' or a.get('assigned_municipality')]
        today = datetime.now(pytz.timezone('Asia/Manila')).date()
...
def get_bfp_causes(time_filter='weekly'):
    try:
        # Load data from fire_incident.csv or .pkl models if available
        if os.path.exists('fire_incident.csv'):
            df = pd.read_csv('fire_incident.csv')
            causes = df['cause'].value_counts().to_dict()
        else:
            # Placeholder data if no CSV or models are available
            causes = {'Electrical': 10, 'Arson': 5, 'Cooking': 8, 'Others': 3}
        return causes
    except Exception as e:
