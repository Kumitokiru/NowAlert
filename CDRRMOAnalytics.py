...
def get_cdrrmo_trends(time_filter='weekly'):
    try:
...
def get_cdrrmo_distribution(time_filter='weekly'):
    try:
        cdrrmo_alerts = [a for a in alerts if a.get('role') == 'cdrrmo' or a.get('assigned_municipality')]
        today = datetime.now(pytz.timezone('Asia/Manila')).date()
...
def get_cdrrmo_causes(time_filter='weekly'):
    try:
        # Placeholder for cause analysis; replace with actual logic if data available
        causes = {'Flood': 10, 'Earthquake': 8, 'Fire': 12, 'Others': 5}
        return causes
    except Exception as e:
